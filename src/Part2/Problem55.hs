...
problem55 :: Int
problem55 = length $ filter (isLychrel 49) [1..9999]
...
isLychrel 0 _ = True
isLychrel rec x = not (isPalindrome x') && isLychrel (rec - 1) x' where
  x' = x + fromDigits (reverse (digits x))
